...
1. 下载干净的 release 包：[html-to-editable-ppt-plugin.zip](https://github.com/JerryLiu-uestc/html-to-editable-ppt/releases/latest/download/html-to-editable-ppt-plugin.zip)。
2. 解压到本地。
3. 在 Codex 中安装或启用本地插件文件夹 `html-to-editable-ppt`。

然后重启 Codex 或刷新插件列表，在 **Personal** marketplace 里启用 **HTML to Editable PPT**。

只有在你想查看示例、demo 素材或开发文件时，才下载源码 ZIP：[source archive](https://github.com/JerryLiu-uestc/html-to-editable-ppt/archive/refs/heads/main.zip)。
...
├── skills/                          # Codex 加载或 Claude Code 阅读的 workflow skill
│   ├── html-to-editable-ppt/
│   ├── frontend-slide-design/
│   └── harness-anything/
├── scripts/                         # 本地 CLI 辅助脚本和 release 打包脚本
│   ├── html_to_editable_ppt.py
│   └── package_release.py
├── docs/                            # CLI 参考文档
├── examples/                        # demo PPTX 和预览图；只在源码仓库中保留
├── README.md / README-zh.md
└── INSTALL.md / INSTALL-zh.md
```

Release ZIP 只包含 `.codex-plugin/`、`skills/`、`scripts/html_to_editable_ppt.py`、`docs/`、README/INSTALL 和 `LICENSE`。它不会包含 `examples/`、`.git/`、`dist/` 或其他开发产物。
